...
export const BeltcutPro: React.FC = () => {
  ...
  return (
    <div className="min-h-full flex flex-col bg-zinc-50/50 text-slate-900 relative">
      {isSyncing && (
            ...
            <div className="xl:col-span-4 space-y-6">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                     ...
                     <div className="flex items-center gap-2">
                        <button onClick={() => setSelectedOrder({...selectedOrder, isInventoryCut: false})} className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${!selectedOrder.isInventoryCut ? 'bg-zinc-950 text-white' : 'bg-slate-100 text-slate-400'}`}>CLIENT</button>
                        <button onClick={() => setSelectedOrder({...selectedOrder, isInventoryCut: true})} className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${selectedOrder.isInventoryCut ? 'bg-zinc-950 text-white' : 'bg-slate-100 text-slate-400'}`}>INVENTORY</button>
                     </div>
                  </div>
                  ...
                  <button onClick={handleCalculateBestFit} className="w-full bg-zinc-900 text-white font-black py-3.5 rounded-xl hover:bg-zinc-800 transition shadow-sm flex items-center justify-center gap-2 text-sm">
                    <Wand2 size={16}/> FIND BEST PIECE
                  </button>
                  <button onClick={() => { setCuttingMode(prev => prev === 'manual' ? 'auto' : 'manual'); }} className="w-full bg-zinc-100 text-zinc-700 font-black py-3 rounded-xl hover:bg-zinc-200 transition text-xs">
              ...
              {currentResult && (
                <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl animate-in slide-in-from-bottom-5">
                   <div className="mb-4 flex justify-between items-center">
                     ...
                     {optimizationResults.length > 1 && cuttingMode === 'auto' && (
                       <div className="flex gap-2">
                         <button onClick={() => setCurrentOptionIndex(prev => (prev - 1 + optimizationResults.length) % optimizationResults.length)} className="p-2 bg-slate-800 rounded-lg"><ChevronLeft size={14}/></button>
                         <button onClick={() => setCurrentOptionIndex(prev => (prev + 1) % optimizationResults.length)} className="p-2 bg-slate-800 rounded-lg"><ChevronRight size={14}/></button>
                       </div>
                     )}
                   </div>
                   ...
                   <button onClick={handleExecuteCut} className="w-full bg-emerald-600 text-white font-black py-3.5 rounded-xl hover:bg-emerald-500 transition shadow-lg active:scale-95 text-xs uppercase tracking-wider">
                     EXECUTE CUT
                   </button>
                </div>
              )}
        ...
        {activeTab === 'stock' && (
          <div className="space-y-8">
            <div className="bg-white p-6 rounded-3xl border border-zinc-200 shadow-sm flex justify-between items-center">
               ...
               <button onClick={() => setShowAddRollForm(!showAddRollForm)} className="px-5 py-3 bg-zinc-950 text-white rounded-xl font-black text-xs hover:bg-zinc-800 transition flex items-center gap-2">
            ...
            {showAddRollForm && (
               <div className="bg-white p-6 rounded-3xl border-2 border-zinc-950 shadow-sm animate-in fade-in slide-in-from-top-3">
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <button onClick={handleAddRoll} className="bg-emerald-600 text-white font-black py-3 rounded-xl hover:bg-emerald-500 transition text-xs">SAVE</button>
                  </div>
               </div>
            )}
